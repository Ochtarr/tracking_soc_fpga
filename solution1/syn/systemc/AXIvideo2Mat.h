// ==============================================================
// RTL generated by Vivado(TM) HLS - High-Level Synthesis from C, C++ and SystemC
// Version: 2017.4.1
// Copyright (C) 1986-2018 Xilinx, Inc. All Rights Reserved.
// 
// ===========================================================

#ifndef _AXIvideo2Mat_HH_
#define _AXIvideo2Mat_HH_

#include "systemc.h"
#include "AESL_pkg.h"


namespace ap_rtl {

struct AXIvideo2Mat : public sc_module {
    // Port declarations 35
    sc_in_clk ap_clk;
    sc_in< sc_logic > ap_rst;
    sc_in< sc_logic > ap_start;
    sc_out< sc_logic > ap_done;
    sc_in< sc_logic > ap_continue;
    sc_out< sc_logic > ap_idle;
    sc_out< sc_logic > ap_ready;
    sc_in< sc_lv<24> > AXI_video_strm_V_data_V;
    sc_in< sc_lv<3> > AXI_video_strm_V_keep_V;
    sc_in< sc_lv<3> > AXI_video_strm_V_strb_V;
    sc_in< sc_lv<1> > AXI_video_strm_V_user_V;
    sc_in< sc_lv<1> > AXI_video_strm_V_last_V;
    sc_in< sc_lv<1> > AXI_video_strm_V_id_V;
    sc_in< sc_lv<1> > AXI_video_strm_V_dest_V;
    sc_in< sc_lv<12> > img_rows_V_dout;
    sc_in< sc_logic > img_rows_V_empty_n;
    sc_out< sc_logic > img_rows_V_read;
    sc_in< sc_lv<11> > img_cols_V_dout;
    sc_in< sc_logic > img_cols_V_empty_n;
    sc_out< sc_logic > img_cols_V_read;
    sc_out< sc_lv<8> > img_data_stream_0_V_din;
    sc_in< sc_logic > img_data_stream_0_V_full_n;
    sc_out< sc_logic > img_data_stream_0_V_write;
    sc_out< sc_lv<8> > img_data_stream_1_V_din;
    sc_in< sc_logic > img_data_stream_1_V_full_n;
    sc_out< sc_logic > img_data_stream_1_V_write;
    sc_out< sc_lv<8> > img_data_stream_2_V_din;
    sc_in< sc_logic > img_data_stream_2_V_full_n;
    sc_out< sc_logic > img_data_stream_2_V_write;
    sc_out< sc_lv<12> > img_rows_V_out_din;
    sc_in< sc_logic > img_rows_V_out_full_n;
    sc_out< sc_logic > img_rows_V_out_write;
    sc_out< sc_lv<11> > img_cols_V_out_din;
    sc_in< sc_logic > img_cols_V_out_full_n;
    sc_out< sc_logic > img_cols_V_out_write;


    // Module declarations
    AXIvideo2Mat(sc_module_name name);
    SC_HAS_PROCESS(AXIvideo2Mat);

    ~AXIvideo2Mat();

    sc_trace_file* mVcdFile;

    sc_signal< sc_logic > ap_done_reg;
    sc_signal< sc_lv<8> > ap_CS_fsm;
    sc_signal< sc_logic > ap_CS_fsm_state1;
    sc_signal< sc_logic > img_rows_V_blk_n;
    sc_signal< sc_logic > img_cols_V_blk_n;
    sc_signal< sc_logic > img_data_stream_0_V_blk_n;
    sc_signal< sc_logic > ap_CS_fsm_pp1_stage0;
    sc_signal< sc_logic > ap_enable_reg_pp1_iter1;
    sc_signal< bool > ap_block_pp1_stage0;
    sc_signal< sc_lv<1> > exitcond_i_reg_436;
    sc_signal< sc_logic > img_data_stream_1_V_blk_n;
    sc_signal< sc_logic > img_data_stream_2_V_blk_n;
    sc_signal< sc_logic > img_rows_V_out_blk_n;
    sc_signal< sc_logic > img_cols_V_out_blk_n;
    sc_signal< sc_lv<1> > eol_reg_206;
    sc_signal< sc_lv<24> > axi_data_V_1_i_reg_217;
    sc_signal< sc_lv<10> > t_V_1_reg_228;
    sc_signal< sc_lv<1> > eol_i_reg_239;
    sc_signal< sc_lv<1> > axi_last_V_2_i_reg_251;
    sc_signal< sc_lv<24> > p_Val2_s_reg_264;
    sc_signal< sc_lv<12> > img_rows_V_read_reg_397;
    sc_signal< bool > ap_block_state1;
    sc_signal< sc_lv<11> > img_cols_V_read_reg_402;
    sc_signal< sc_lv<24> > tmp_data_V_reg_407;
    sc_signal< sc_logic > ap_CS_fsm_state2;
    sc_signal< sc_lv<1> > tmp_last_V_reg_415;
    sc_signal< sc_lv<1> > exitcond1_i_fu_333_p2;
    sc_signal< sc_logic > ap_CS_fsm_state4;
    sc_signal< sc_lv<11> > i_V_fu_338_p2;
    sc_signal< sc_lv<11> > i_V_reg_431;
    sc_signal< sc_lv<1> > exitcond_i_fu_348_p2;
    sc_signal< bool > ap_block_state5_pp1_stage0_iter0;
    sc_signal< bool > ap_block_state6_pp1_stage0_iter1;
    sc_signal< bool > ap_block_pp1_stage0_11001;
    sc_signal< sc_lv<10> > j_V_fu_353_p2;
    sc_signal< sc_logic > ap_enable_reg_pp1_iter0;
    sc_signal< sc_lv<1> > brmerge_i_fu_362_p2;
    sc_signal< sc_lv<8> > tmp_fu_368_p1;
    sc_signal< sc_lv<8> > tmp_reg_449;
    sc_signal< sc_lv<8> > tmp_2_reg_454;
    sc_signal< sc_lv<8> > tmp_3_reg_459;
    sc_signal< sc_logic > ap_CS_fsm_state8;
    sc_signal< sc_lv<1> > ap_phi_mux_eol_2_i_phi_fu_303_p4;
    sc_signal< bool > ap_block_pp1_stage0_subdone;
    sc_signal< sc_logic > ap_condition_pp1_exit_iter0_state5;
    sc_signal< sc_lv<1> > axi_last_V_3_i_reg_276;
    sc_signal< sc_lv<1> > axi_last_V1_i_reg_175;
    sc_signal< sc_logic > ap_CS_fsm_state9;
    sc_signal< sc_logic > ap_CS_fsm_state3;
    sc_signal< sc_lv<24> > axi_data_V_3_i_reg_288;
    sc_signal< sc_lv<24> > axi_data_V1_i_reg_185;
    sc_signal< sc_lv<11> > t_V_reg_195;
    sc_signal< sc_lv<1> > ap_phi_mux_eol_phi_fu_209_p4;
    sc_signal< sc_lv<24> > ap_phi_mux_axi_data_V_1_i_phi_fu_220_p4;
    sc_signal< sc_lv<1> > ap_phi_mux_eol_i_phi_fu_243_p4;
    sc_signal< sc_lv<1> > ap_phi_reg_pp1_iter0_axi_last_V_2_i_reg_251;
    sc_signal< sc_lv<24> > ap_phi_mux_p_Val2_s_phi_fu_268_p4;
    sc_signal< sc_lv<24> > ap_phi_reg_pp1_iter0_p_Val2_s_reg_264;
    sc_signal< sc_logic > ap_CS_fsm_state7;
    sc_signal< sc_lv<1> > eol_2_i_reg_300;
    sc_signal< bool > ap_block_pp1_stage0_01001;
    sc_signal< sc_lv<1> > sof_1_i_fu_104;
    sc_signal< sc_lv<12> > t_V_cast_i_fu_329_p1;
    sc_signal< sc_lv<11> > t_V_1_cast_i_fu_344_p1;
    sc_signal< sc_lv<1> > tmp_user_V_fu_320_p1;
    sc_signal< sc_lv<8> > ap_NS_fsm;
    sc_signal< sc_logic > ap_idle_pp1;
    sc_signal< sc_logic > ap_enable_pp1;
    sc_signal< bool > ap_condition_249;
    sc_signal< bool > ap_condition_159;
    static const sc_logic ap_const_logic_1;
    static const sc_logic ap_const_logic_0;
    static const sc_lv<8> ap_ST_fsm_state1;
    static const sc_lv<8> ap_ST_fsm_state2;
    static const sc_lv<8> ap_ST_fsm_state3;
    static const sc_lv<8> ap_ST_fsm_state4;
    static const sc_lv<8> ap_ST_fsm_pp1_stage0;
    static const sc_lv<8> ap_ST_fsm_state7;
    static const sc_lv<8> ap_ST_fsm_state8;
    static const sc_lv<8> ap_ST_fsm_state9;
    static const sc_lv<32> ap_const_lv32_0;
    static const bool ap_const_boolean_1;
    static const sc_lv<32> ap_const_lv32_4;
    static const bool ap_const_boolean_0;
    static const sc_lv<1> ap_const_lv1_0;
    static const sc_lv<32> ap_const_lv32_1;
    static const sc_lv<32> ap_const_lv32_3;
    static const sc_lv<32> ap_const_lv32_6;
    static const sc_lv<1> ap_const_lv1_1;
    static const sc_lv<32> ap_const_lv32_7;
    static const sc_lv<32> ap_const_lv32_2;
    static const sc_lv<11> ap_const_lv11_0;
    static const sc_lv<10> ap_const_lv10_0;
    static const sc_lv<32> ap_const_lv32_5;
    static const sc_lv<11> ap_const_lv11_1;
    static const sc_lv<10> ap_const_lv10_1;
    static const sc_lv<32> ap_const_lv32_8;
    static const sc_lv<32> ap_const_lv32_F;
    static const sc_lv<32> ap_const_lv32_10;
    static const sc_lv<32> ap_const_lv32_17;
    // Thread declarations
    void thread_ap_clk_no_reset_();
    void thread_ap_CS_fsm_pp1_stage0();
    void thread_ap_CS_fsm_state1();
    void thread_ap_CS_fsm_state2();
    void thread_ap_CS_fsm_state3();
    void thread_ap_CS_fsm_state4();
    void thread_ap_CS_fsm_state7();
    void thread_ap_CS_fsm_state8();
    void thread_ap_CS_fsm_state9();
    void thread_ap_block_pp1_stage0();
    void thread_ap_block_pp1_stage0_01001();
    void thread_ap_block_pp1_stage0_11001();
    void thread_ap_block_pp1_stage0_subdone();
    void thread_ap_block_state1();
    void thread_ap_block_state5_pp1_stage0_iter0();
    void thread_ap_block_state6_pp1_stage0_iter1();
    void thread_ap_condition_159();
    void thread_ap_condition_249();
    void thread_ap_condition_pp1_exit_iter0_state5();
    void thread_ap_done();
    void thread_ap_enable_pp1();
    void thread_ap_idle();
    void thread_ap_idle_pp1();
    void thread_ap_phi_mux_axi_data_V_1_i_phi_fu_220_p4();
    void thread_ap_phi_mux_eol_2_i_phi_fu_303_p4();
    void thread_ap_phi_mux_eol_i_phi_fu_243_p4();
    void thread_ap_phi_mux_eol_phi_fu_209_p4();
    void thread_ap_phi_mux_p_Val2_s_phi_fu_268_p4();
    void thread_ap_phi_reg_pp1_iter0_axi_last_V_2_i_reg_251();
    void thread_ap_phi_reg_pp1_iter0_p_Val2_s_reg_264();
    void thread_ap_ready();
    void thread_brmerge_i_fu_362_p2();
    void thread_exitcond1_i_fu_333_p2();
    void thread_exitcond_i_fu_348_p2();
    void thread_i_V_fu_338_p2();
    void thread_img_cols_V_blk_n();
    void thread_img_cols_V_out_blk_n();
    void thread_img_cols_V_out_din();
    void thread_img_cols_V_out_write();
    void thread_img_cols_V_read();
    void thread_img_data_stream_0_V_blk_n();
    void thread_img_data_stream_0_V_din();
    void thread_img_data_stream_0_V_write();
    void thread_img_data_stream_1_V_blk_n();
    void thread_img_data_stream_1_V_din();
    void thread_img_data_stream_1_V_write();
    void thread_img_data_stream_2_V_blk_n();
    void thread_img_data_stream_2_V_din();
    void thread_img_data_stream_2_V_write();
    void thread_img_rows_V_blk_n();
    void thread_img_rows_V_out_blk_n();
    void thread_img_rows_V_out_din();
    void thread_img_rows_V_out_write();
    void thread_img_rows_V_read();
    void thread_j_V_fu_353_p2();
    void thread_t_V_1_cast_i_fu_344_p1();
    void thread_t_V_cast_i_fu_329_p1();
    void thread_tmp_fu_368_p1();
    void thread_tmp_user_V_fu_320_p1();
    void thread_ap_NS_fsm();
};

}

using namespace ap_rtl;

#endif
